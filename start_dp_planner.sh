...
LAUNCH_FILE="${ROOT_DIR}/basic_dev/src/controller/launch/hybrid_dp_control.launch"
LOG_DIR="${ROOT_DIR}/logs"

SIM_LOG="${LOG_DIR}/dp_simulator.log"
LAUNCH_LOG="${LOG_DIR}/dp_control.log"
TAKEOFF_LOG="${LOG_DIR}/dp_takeoff.log"
POST_TAKEOFF_WAIT_SEC="${POST_TAKEOFF_WAIT_SEC:-2}"
SIM_SEED="${SIM_SEED:-123}"
...
start_simulator() {
  info "启动模拟器: ${SIM_SCRIPT} (seed=${SIM_SEED})"
  (
    cd "${SIM_DIR}" || exit 1
    bash "${SIM_SCRIPT}" "${SIM_SEED}"
  ) >"${SIM_LOG}" 2>&1 &
  SIM_PID=$!
...
start_stack() {
  info "启动混合规划控制链路: ${LAUNCH_FILE}"
  (
    cd "${WS_DIR}" || exit 1
    source "${ROS_SETUP}"
    source "${CATKIN_SETUP}"
    roslaunch controller hybrid_dp_control.launch
  ) >"${LAUNCH_LOG}" 2>&1 &
  LAUNCH_PID=$!
  info "混合规划控制链路已启动，pid=${LAUNCH_PID}，日志: ${LAUNCH_LOG}"
}
...
main() {
  ...
  info "等待起飞动作稳定 ${POST_TAKEOFF_WAIT_SEC}s，再启动混合规划控制链路。"
  ...
  if ! check_component_alive "${LAUNCH_PID}" "混合规划控制链路 launch" "${LAUNCH_LOG}"; then
    exit 1
  fi

  info "混合规划链路已全部启动完成。"
  info "模拟器日志: ${SIM_LOG}"
  info "DP 控制日志: ${LAUNCH_LOG}"
